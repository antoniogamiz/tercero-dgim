...
class afd:
    ...
    def checkDeterminist(self):
        ...
        for q in self.states:
            for c in self.alphabet:
                t = self.getValidTransitions(c, q)
                if len(t) == 0:
                    if self.allowImplicitReject:
                        return "NO"
                        if self.verbose:
                            print ( "There is not transition for character %s", c )
                    else:
                        return "E"
                        print ( "Error, Automaton with more than a transition applicable %s ", c )
        return True

    def _generateDescription(self, states, final, transitions):
        """
        It generates a string containing the automaton 
        in the format described above.
        
        Args:
            states (vector str): contains the automaton states.
            final (vector str): contains the automaton final states.
            transitions (dict): key: state value: transitions of that state.
    ...
    def _automatonProduct(self, aut2):
        """
        If the automaton is Deterministic, it calculates the states
        and the transitions of the Automaton Product.

        Args: 
            aut2 (afd): automaton to calculate the product
        
        Return: vector containing states and transitions of the Automaton Product.
        ...
        states.append( (self.states[0] , aut2.states[0]) )
        ...
        while len( queue ) > 0 :
            ...
            for c in self.alphabet:
                left = self.getValidTransitions(c, q[0])[0]
                right = aut2.getValidTransitions(c, q[1])[0]

                nextState = (left.destState , right.destState)
                trans = Transition( q[0]+q[1], nextState[0]+nextState[1], c )
                transitions[ q[0]+q[1] ].append( trans )
                
                if nextState not in states:
                    states.append( nextState )
                    queue.append( nextState )
        
        statesMod = [ q[0]+q[1] for q in states ] 

        return [ statesMod, transitions ]            

    def intersection(self, aut2, filename="intersection.af"):
        """
        It calculates the intersection of the automaton <self> and <aut2>
        and stores it in <filename>.

        Args:
            aut2 (afd): automaton used to calculate the intersection.
            filename (str): name of the file where the intersection will be stored.
        """
        if not self.checkDeterminist() or not aut2.checkDeterminist:
            print( "Error: automatons have to be deterministic." )
            return

        states, transitions = self._automatonProduct(aut2)
        finals = []
        
        for q1 in self.final:
            for q2 in aut2.final:
                if q1+q2 in states:
                    finals.append( q1+q2 )

        description = self._generateDescription( states, finals, transitions )
        wf(filename, description)
        print("Automaton generated in " + filename)

    def union(self, aut2, filename="union.af"):
        """
        It calculates the union of the automaton <self> and <aut2>
        and stores it in <filename>.

        Args:
            aut2 (afd): automaton used to calculate the union.
            filename (str): name of the file where the union will be stored.
        """
        ...
        for q1 in self.states:
            for q2 in aut2.final:
                if q1+q2 in states and q1+q2 not in finals:
                    finals.append( q1+q2 )
    ...
    def complementary(self, filename="complementary.af"):
        """
        It calculates the complementary of the automaton <self> and <aut2>
        and stores it in <filename>.

        Args:
            aut2 (afd): automaton used to calculate the complementary.
            filename (str): name of the file where the complementary will be stored.
        """

        if not self.checkDeterminist():
            print( "Error: automatons have to be deterministic." )
            return
        finals = []
        for q in self.states:
            if q not in self.final:
                finals.append( q )
        description = self._generateDescription( self.states, finals, self.transitions )
        wf(filename, description)
        print("Automaton generated in " + filename)

    def _accessibleStates(self):
        """
        It calculates the accessible states.

        Return: a sorted vector containing the accessible states.
        """
        ...
        while len( queue ) > 0:
            ...
            for c in self.alphabet:
                t = self.getValidTransitions(c, current)[0]
                if t.destState not in states:
                    states.append( t.destState )
                    queue.append( t.destState )
        return sorted(states)

    def _getIndex(self, coupleList, couple):
        """
        It calculates the index of <couple> in <coupleList>.

        Return: int index
        """
        index = None

        for i in range(0, len(coupleList) ):
            if couple == coupleList[i][0]:
                index = i

        if index == None:
            couple = [ x for x in reversed(couple) ]
            for i in range(0, len(coupleList) ):
                if couple == coupleList[i][0]:
                    index = i
    ...
    def minimize(self, filename="minimize.af"):
        """
        It minimizes the automaton and stores the result in <filename>.

        Args: 
            filename (str): name of the file where the minimized automaton will be stored.
        """
        states = self._accessibleStates()

        coupleStates = []
        for i in range(1, len(states)):
            for j in range(0, i):   
                coupleStates.append( [ [states[i], states[j]] , False, [] ] )
        
        for p in coupleStates:
            if p[0][0] in self.final and p[0][1] not in self.final:
                p[1] = True
            if p[0][0] not in self.final and p[0][1] in self.final:
                p[1] = True

        for p in coupleStates:
            for c in self.alphabet:
                left = self.getValidTransitions(c, p[0][0])[0].destState
                right = self.getValidTransitions(c, p[0][1])[0].destState

                if left != right:
                    index = self._getIndex( coupleStates, [left,right] )
                    if coupleStates[index][1]:
                        p[1] = True
                        for k in p[2]:
                            k[1] = True
                    else:
                        coupleStates[index][2].append( p )

        notMarked = []
        for p in coupleStates:
            if not p[1]:
                notMarked.append( p[0] )

        for p in notMarked:
            self.states.pop( self.states.index( p[0] ) )
            self.transitions.pop(p[0], None)
            for tList in self.transitions.values():
                for t in tList:
                    if t.destState == p[0]:
                        t.destState = p[1]
