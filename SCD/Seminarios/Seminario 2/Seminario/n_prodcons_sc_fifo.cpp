...
// g++ -std=c++11 -pthread -I. -o exe N_Productores_N_Consumidores_LIFO.cpp Semaphore.cpp
...
class ProdConsNSC
{
 ...
 int                        // variables permanentes
   ...
   lectura, escritura ;          //  indice de celda de la próxima inserción
 mutex
...
int ProdConsNSC::leer(  )
{
   ...
   // hacer la operación de lectura, actualizando el estado del monitor
   const int valor = buffer[lectura] ;
   lectura = (lectura + 1) % num_celdas_total;
   if ( lectura == escritura ) vacio = true;
   lleno=false;
...
void ProdConsNSC::escribir( int valor )
{
   ...
   if ( lectura == escritura ) lleno = true;
   vacio = false;
...
void funcion_hebra_productora( ProdConsNSC * monitor, int num_hebra)
{
...
void funcion_hebra_consumidora( ProdConsNSC * monitor, int num_hebra )
{
