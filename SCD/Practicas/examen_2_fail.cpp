...
void funcion_consumidor( int id_propio )
{
   ...
   int tag = ( id_propio % 2 == 0 ) ? etiq_buf_par : etiq_buf_impar;
...
void funcion_buffer()
{
   ...
   while( i < num_items )
   {
       sleep_for ( milliseconds( 100 ) );
        ...
        // cout << "flag: " << flag << endl;
        if ( (num_celdas_ocupadas < tam_vector) && flag) {
        ...
        if( num_celdas_ocupadas && flag) {
            // cout << "Flag cons: " << flag << endl;
            if( ( (valor % 2 == 1) && (estado.MPI_SOURCE % 2 == 1) ) || ( (valor % 2 == 0) && (estado.MPI_SOURCE % 2 == 0) )) {
                MPI_Recv( &valor, 1, MPI_INT, estado.MPI_SOURCE, etiq_cons, MPI_COMM_WORLD, &estado );
                valor = buffer[primera_ocupada];
                if( (valor % 2 == 0) && (estado.MPI_SOURCE % 2 == 0) )
                    MPI_Ssend( &valor, 1, MPI_INT, estado.MPI_SOURCE, etiq_buf_par, MPI_COMM_WORLD);
                else if( (valor % 2 == 1) && (estado.MPI_SOURCE % 2 == 1) )
                    MPI_Ssend( &valor, 1, MPI_INT, estado.MPI_SOURCE, etiq_buf_impar, MPI_COMM_WORLD);      
