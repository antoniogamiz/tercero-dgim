...
int MODE = 1; // inicialmente esta en modo 1
...
{
    int tag = ( id_propio % 2 == 0 ) ? etiq_prod_par : etiq_prod_impar;
   for ( unsigned int i= 0 ; i < num_items_prod ; i++ )
   {
      ...
      MPI_Ssend( &valor_prod, 1, MPI_INT, id_buffer, tag, MPI_COMM_WORLD );
   }
}
// ---------------------------------------------------------------------
...
void funcion_buffer()
{
   ...
   while( i < num_items )
   {
      ...
      if ( num_celdas_ocupadas == 0 ) {
         tag = ( MODE == 1 ) ? etiq_prod_impar : etiq_prod_par;       // $~~~$ solo prod.
         cout << " d" << tag << " " << i<< endl;
      }           // si buffer vacío
    ...
    //      tag = MPI_ANY_TAG ;     // $~~~$ cualquiera
      ...
      //Manejamos el recibimiento de un mensaje por parte de un productor
      if( tag == etiq_prod_par || tag == etiq_prod_impar ) {
          MPI_Iprobe(MPI_ANY_SOURCE, tag, MPI_COMM_WORLD, &flag, &estado);
          if( flag ) {
                MPI_Recv( &valor, 1, MPI_INT, estado.MPI_SOURCE, tag, MPI_COMM_WORLD, &estado );
