...
void funcion_buffer()
{
   ...
   for( unsigned int i=0 ; i < num_items*2 ; i++ )
   {
      ...
      if ( num_celdas_ocupadas > 0 )               // si buffer vacío
         tag = etiq_cons;       // $~~~$ solo prod.
      else 
         tag = etiq_prod ;      // $~~~$ solo cons.
      ...
      if( tag == etiq_prod ) {
          j++;
          if( MODE == 1 ) { // impares
              MPI_Probe(MPI_ANY_SOURCE, etiq_prod_impar, MPI_COMM_WORLD, &estado);
              cout << "antes" << endl;
              MPI_Recv( &valor, 1, MPI_INT, MPI_ANY_SOURCE, etiq_prod_impar, MPI_COMM_WORLD, &estado );
              cout << "despues" << endl;
              MODE = 2;   
          } else { // pares
              MPI_Probe(MPI_ANY_SOURCE, etiq_prod_par, MPI_COMM_WORLD, &estado);
              MPI_Recv( &valor, 1, MPI_INT, MPI_ANY_SOURCE, etiq_prod_par, MPI_COMM_WORLD, &estado );
              MODE = 1;
          }
      }
